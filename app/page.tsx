...
interface Prompt {
  ...
  category: string; tags: string[]; compat: string[];
  diff: "Básico" | "Intermedio" | "Avanzado";
  pop: number; featured?: boolean; trending?: boolean;
  genderMode?: boolean; previewF?: string; _search?: string;
}

interface AITool {
  id: string; name: string; specialty: string; badge: string;
  badgeColor: string; url: string; color: string; bg: string; categories: string[];
}
...
const TOP_PROMPTS = PROMPTS.filter(p => p.trending || p.featured).sort((a,b) => b.pop - a.pop).slice(0, 6);
  ...
  if (state === "menu") return (
    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
      {["ChatGPT","Claude","Gemini","Perplexity"].map(ai => {
        ...
        return (
          <button key={ai} onClick={() => openInAI(ai)} aria-label={`Abrir en ${ai}`}
      ...
      <button onClick={() => setState("idle")} aria-label="Cerrar"
        style={{padding:"5px 10px",borderRadius:8,fontSize:11,border:"1px solid rgba(255,255,255,0.1)",background:"transparent",color:"#555"}}>✕</button>
    </div>
  );

  return (
    <div style={{display:"flex",gap:6}}>
      <button className="copy-btn" onClick={() => { navigator.clipboard.writeText(text); setState("copied"); setTimeout(() => setState("idle"), 2000); }}
        aria-label={state === "copied" ? "Copiado al portapapeles" : "Copiar prompt"} aria-live="polite"
        style={{padding:"7px 14px",borderRadius:8,fontSize:12,fontWeight:600,border:`1px solid ${state==="copied"?"rgba(74,222,128,0.4)":"rgba(99,102,241,0.3)"}`,background:state==="copied"?"rgba(74,222,128,0.1)":"rgba(99,102,241,0.1)",color:state==="copied"?"#4ade80":"#818cf8"}}>
        {state==="copied"?"✓ Copiado":"Copiar"}
      </button>
      <button className="copy-btn" onClick={() => setState("menu")} aria-label="Abrir en IA"
        style={{padding:"7px 10px",borderRadius:8,fontSize:12,fontWeight:600,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.03)",color:"#555"}}>↗</button>
    </div>
  );
}

function PromptCard({ p, gender }: { p: Prompt; gender: Gender }) {
    ...
    <article className="card" style={{borderRadius:16,padding:22,border:"1px solid rgba(255,255,255,0.05)",background:"linear-gradient(145deg,rgba(255,255,255,0.025),rgba(255,255,255,0.01))",display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          <span aria-hidden="true" style={{fontSize:14,opacity:0.6}}>{catIcon}</span>
          <span style={{fontSize:10,color:"#444",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",fontFamily:"JetBrains Mono,monospace"}}>{catLabel}</span>
          {p.genderMode && <span aria-label="Disponible para hombre y mujer" style={{fontSize:10,color:"#555"}}>♂♀</span>}
          {p.trending && <span style={{fontSize:9,padding:"2px 6px",borderRadius:4,background:"rgba(251,191,36,0.12)",border:"1px solid rgba(251,191,36,0.3)",color:"#fbbf24",fontWeight:700}}>TRENDING</span>}
          {p.featured && !p.trending && <span style={{fontSize:9,padding:"2px 6px",borderRadius:4,background:"rgba(99,102,241,0.12)",border:"1px solid rgba(99,102,241,0.3)",color:"#818cf8",fontWeight:700}}>FEATURED</span>}
        </div>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <span style={{fontSize:10,color:DIFF_C[p.diff],border:`1px solid ${DIFF_C[p.diff]}33`,padding:"2px 7px",borderRadius:4,fontWeight:700}}>{p.diff}</span>
          <span aria-label={`Popularidad: ${p.pop}`} style={{fontSize:10,color:"#333",fontFamily:"JetBrains Mono,monospace"}}>★{p.pop}</span>
        </div>
      </div>
      <div>
      ...
      <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
        {p.tags.map(t => <span key={t} style={{fontSize:10,padding:"2px 8px",borderRadius:5,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",color:"#444",fontFamily:"JetBrains Mono,monospace"}}>#{t}</span>)}
      </div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"auto"}}>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {p.compat.slice(0,3).map(ai => {
            const c = AI_COMPAT[ai] || {color:"#888",bg:"rgba(136,136,136,0.1)"};
            return <span key={ai} style={{fontSize:10,padding:"2px 7px",borderRadius:20,background:c.bg,color:c.color,border:`1px solid ${c.color}25`,fontWeight:600}}>{ai}</span>;
          })}
        </div>
        ...
        <header style={{position:"sticky",top:0,zIndex:200,borderBottom:"1px solid rgba(255,255,255,0.05)",background:"rgba(5,5,7,0.85)",backdropFilter:"blur(24px)"}}>
          <div style={{maxWidth:1280,margin:"0 auto",padding:"0 24px",height:58,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div aria-hidden="true" style={{width:30,height:30,borderRadius:8,background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:900,color:"#fff"}}>P</div>
              <span style={{fontSize:14,fontWeight:800,letterSpacing:"-0.02em",color:"#f0f0f4"}}>PROMT<span style={{color:"#6366f1"}}>LAB</span></span>
              <span style={{fontSize:9,padding:"2px 6px",borderRadius:4,background:"rgba(99,102,241,0.15)",border:"1px solid rgba(99,102,241,0.3)",color:"#818cf8",fontWeight:700,letterSpacing:"0.05em"}}>v2.1</span>
            </div>
            <nav style={{display:"flex",gap:4}} aria-label="Navegación principal">
              {[["prompts","Prompts"],["top","Top 10"],["ias","Mejores IAs"]].map(([id,label]) => (
                <button key={id} onClick={() => setActiveSection(id as "prompts"|"ias"|"top")} className="tab-btn"
            ...
            <button style={{padding:"7px 16px",borderRadius:8,background:"linear-gradient(135deg,#6366f1,#7c3aed)",color:"#fff",border:"none",fontSize:13,fontWeight:600}}>
              Registro gratis
            </button>
          </div>
        </header>
        ...
        <section className="grid-bg" style={{position:"relative",overflow:"hidden",padding:"80px 24px 60px",textAlign:"center",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
          ...
          <div style={{position:"relative",maxWidth:760,margin:"0 auto"}}>
            ...
            <div className="fu4" style={{position:"relative",maxWidth:580,margin:"0 auto 28px"}}>
              <span aria-hidden="true" style={{position:"absolute",left:18,top:"50%",transform:"translateY(-50%)",color:"#333",fontSize:18,pointerEvents:"none"}}>⌕</span>
              <input type="search" value={query} onChange={e => setQuery(e.target.value)}
                placeholder="Buscar prompts, categorías, efectos..." aria-label="Buscar prompts"
              ...
              {query && <button onClick={() => setQuery("")} aria-label="Limpiar búsqueda" style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#333",fontSize:16}}>✕</button>}
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"center"}} role="group" aria-label="Filtro por género">
              {[["general","✦ General"],["hombre","♂ Hombre"],["mujer","♀ Mujer"]].map(([id,label]) => (
                <button key={id} onClick={() => setGender(id as Gender)} aria-pressed={gender===id}
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:16}}>
              {TOP_PROMPTS.map((p, i) => (
                <div key={p.id} style={{position:"relative"}}>
                  <div aria-label={`Posición ${i+1}`} style={{position:"absolute",top:-10,left:16,zIndex:10,width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:900,color:"#fff",border:"2px solid #050507"}}>{i+1}</div>
                  <PromptCard p={p} gender={gender} />
                </div>
              ))}
            </div>
          </section>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
              {AI_TOOLS.map(ai => (
                <article key={ai.id} className="ai-card" style={{borderRadius:16,padding:22,border:"1px solid rgba(255,255,255,0.05)",background:`linear-gradient(145deg,${ai.bg},rgba(255,255,255,0.01))`,display:"flex",flexDirection:"column",gap:14}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div aria-hidden="true" style={{width:42,height:42,borderRadius:12,background:ai.bg,border:`1px solid ${ai.color}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:900,color:ai.color}}>{ai.name[0]}</div>
                    <span style={{fontSize:9,padding:"3px 8px",borderRadius:20,background:`${ai.badgeColor}18`,border:`1px solid ${ai.badgeColor}33`,color:ai.badgeColor,fontWeight:700,letterSpacing:"0.03em"}}>{ai.badge}</span>
                  </div>
                  <div>
                  ...
                  <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                    {ai.categories.slice(0,3).map(cat => { const c = CATS.find(c => c.id === cat); return c ? <span key={cat} style={{fontSize:9,padding:"2px 7px",borderRadius:4,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)",color:"#444"}}>{c.label}</span> : null; })}
                  </div>
          ...
          <>
            <section style={{padding:"28px 24px 16px",maxWidth:1280,margin:"0 auto"}}>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                {CATS.map(c => (
                  <button key={c.id} className="cat-btn" onClick={() => setActiveCat(c.id)} aria-pressed={activeCat===c.id}
                    ...
                    <span aria-hidden="true" style={{fontSize:11}}>{c.icon}</span>{c.label}
                  </button>
                ))}
                <span aria-live="polite" style={{marginLeft:"auto",fontSize:11,color:"#333",fontFamily:"JetBrains Mono,monospace"}}>{filtered.length} prompts</span>
              </div>
            </section>
            <section style={{padding:"12px 24px 80px",maxWidth:1280,margin:"0 auto"}}>
              {filtered.length === 0 ? (
                <div style={{textAlign:"center",padding:"80px 0"}}>
                  <div aria-hidden="true" style={{fontSize:40,marginBottom:16,opacity:0.2}}>◈</div>
                  <p style={{color:"#333",fontSize:16,fontWeight:600}}>No encontramos prompts para "{query}"</p>
                  <button onClick={() => {setQuery("");setActiveCat("all");}} style={{marginTop:16,padding:"8px 20px",borderRadius:8,border:"1px solid rgba(99,102,241,0.3)",background:"rgba(99,102,241,0.1)",color:"#818cf8",fontSize:13}}>Limpiar filtros</button>
                </div>
              ) : (
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
                  {filtered.map(p => <PromptCard key={p.id} p={p} gender={gender} />)}
                </div>
              )}
            </section>
          </>
          ...
          <div style={{maxWidth:1280,margin:"0 auto"}}>
            <div style={{borderRadius:20,padding:"56px 48px",textAlign:"center",background:"linear-gradient(135deg,rgba(99,102,241,0.1),rgba(139,92,246,0.06))",border:"1px solid rgba(99,102,241,0.18)",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{position:"relative"}}>
                ...
                <h2 style={{fontSize:"clamp(24px,3.5vw,40px)",fontWeight:900,letterSpacing:"-0.04em",marginBottom:14,color:"#f5f5fa"}}>Acceso ilimitado.<br />Sin restricciones.</h2>
                <p style={{color:"#444",fontSize:15,marginBottom:28,maxWidth:440,margin:"0 auto 28px",lineHeight:1.7}}>Desbloquea cientos de prompts exclusivos, colecciones premium y actualizaciones semanales.</p>
                <button style={{padding:"13px 32px",borderRadius:12,fontSize:15,fontWeight:700,background:"linear-gradient(135deg,#6366f1,#7c3aed)",color:"#fff",border:"none"}}>Comenzar gratis →</button>
              </div>
            </div>
          </div>
        </section>
        ...
        <footer style={{borderTop:"1px solid rgba(255,255,255,0.04)",padding:"32px 24px"}}>
          <div style={{maxWidth:1280,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div aria-hidden="true" style={{width:24,height:24,borderRadius:6,background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:900,color:"#fff"}}>P</div>
              <span style={{fontSize:13,fontWeight:700,color:"#333"}}>PROMTLAB</span>
            </div>
            <nav style={{display:"flex",gap:24}} aria-label="Enlaces legales">
              {["Términos","Privacidad","Contacto"].map(l => <a key={l} href="#" style={{fontSize:12,color:"#333",textDecoration:"none"}}>{l}</a>)}
            </nav>
